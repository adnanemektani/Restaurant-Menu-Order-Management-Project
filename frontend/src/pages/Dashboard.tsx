...
export default function Dashboard() {
 ...
 const fetchOrders = async () => {
  ...
  const statusConfig: any = {
    new: { label: 'New', line: 'bg-stone-800', btnColor: 'border-stone-600 text-stone-700 hover:bg-stone-800 hover:text-white' },
    preparing: { label: 'Preparing', line: 'bg-amber-700', btnColor: 'border-amber-700 text-amber-800 hover:bg-amber-700 hover:text-white' },
    ready: { label: 'Ready', line: 'bg-blue-800', btnColor: 'border-blue-700 text-blue-800 hover:bg-blue-800 hover:text-white' },
    delivered: { label: 'Delivered', line: 'bg-green-800', btnColor: 'border-green-700 text-green-800 hover:bg-green-800 hover:text-white' },
  }
      ...
      <div className="pt-20 pb-12 px-8 flex-1">
        ...
        <div className="py-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Pending', value: orders.filter(o => o.status === 'new').length },
            { label: 'In Progress', value: orders.filter(o => o.status === 'preparing').length },
          ...
          {['new', 'preparing', 'ready', 'delivered'].map(status => (
            <div key={status}
              ...
              <div className="p-4 text-center">
                <h2 className="text-lg font-semibold text-stone-800 uppercase tracking-widest"
                  style={{ fontFamily: 'Playfair Display, serif' }}>
                ...
                {orders.filter(o => o.status === status).map(order => (
                  <div key={order.id}
                    ...
                    <div className="flex justify-between items-center mb-1">
                      <span className="font-bold text-stone-800">Table {order.table_number}</span>
                      <span className="text-amber-800 font-semibold text-sm">{order.total} DH</span>
                    </div>
